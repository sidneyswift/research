...
- `#rule-engine` — root `AGENTS.md`/`CLAUDE.md` (synced byte-for-byte by `bin/sync-claude-md.sh`; CI `--check` gate; real file not symlink "to survive cross-platform ZIP extraction"): mandatory pre-flight self-report block (Context files ✓/✗, routing declared, "Deliverable requested explicitly? Y/N — if N, no draft output permitted"), 5 non-negotiable rules (read Context first; route via pm-workflows; gate question before deliverables; journalist/spy rule — ask, don't tell; cite a Knowledge file before any PM opinion), Context Guard placeholder sentinel (`[Company name]` present → hard-stop to `/start`; `/dev` disables only the guard), scope-refusal script line; cursor variant ships the same text as an `alwaysApply` `.cursor/rules/pm-os-core-rules.mdc` self-described as "compaction-survival anchor"
- `#ci-validators` — `.github/workflows/ci.yml` + `release.yml` and the 15 shipped `bin/` validators: registry-drift `--check` regeneration gates, skill cross-reference validation, skill-category coverage (empty-bucket check), changelog-freshness guard, `check-no-hardcoded-versions.sh` (with a documented doc-rot postmortem in its header), AGENTS↔CLAUDE sync check; tag-push release builds the 3 zips behind these gates. All **structural** validation — no behavioral skill tests, LLM evals, or routing evals anywhere
- `#onboarding` — `pm-os-start/SKILL.md` (469 lines): three-way entry menu (set up / import from Claude-ChatGPT-Gemini / just demo via `/help`), 4 phases using each platform's native structured-question tool (named per surface, with batching rules), web-search-then-confirm drafts ("Here's what I found about [Company]. Tell me what's wrong or say 'looks good'"), progress echoes ("Got it — [Company] is set. 3 steps left"), per-phase telemetry pings
- `#upgrade-skill` — `pm-os-upgrade/SKILL.md` + `bin/upgrade.sh`: tool-aware workspace detection (cursor / claude-code / mixed-v2.0-era), wrong-ZIP refusal with friendly message + exit code 2 and zero files modified, `--dry-run` first, explicit preserved-vs-replaced path lists (user Context/Work/custom skills preserved; system dirs replaced)
- `#agent-classification` — `registry/agent-classification.md`: explicit triage "so PM OS does not create decorative subagents" — active subagents (isolated context + bounded output earns it) vs router prompts (AGENTS.md, `.mdc` rules, command stubs) vs skill-like/deferred (with rationale for each deferral)
