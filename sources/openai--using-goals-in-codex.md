...
- [[sources/openai--plugins]] — **the other OpenAI/Codex source.** This guide is the Codex *behavioral* surface (how a long objective runs); `openai/plugins` is the Codex *packaging/distribution* surface (how Codex gains capabilities). Same creator, same CLI — together they give the wiki two layers of Codex.
- [[creators/openai]] — the publisher (this page seeded OpenAI's first creator entry; now joined by [[sources/openai--plugins]]).
