...
> **Confirmed 2026-06-01 — with a standing caveat.** Two distinct in-wiki artifacts demonstrate the full tested bundle: gstack (the 7 components as real files) and gbrain (`gbrain doctor` enforcing all 10 steps). **The caveat: both are the same creator (Garry Tan)** — this is "his house style" demonstrated across two of his repos, not yet a cross-ecosystem convention. The strongest next evidence would be a **non-Garry artifact** that ships the full bundle (especially the resolver eval, the rare part); [[artifacts/plugins/anthropic-financial-services-marketplace]] ships *output* evals but not visibly *resolver* evals. Re-examine if that caveat ever sharpens into a counter-example.
