...
- *(supporting — Codex-ecosystem eval-tooling sighting, not a verified full per-skill bundle)* [[artifacts/plugins/openai-codex-plugins-marketplace]] — OpenAI's `plugin-eval` is a **meta-plugin whose job is to score other Codex skills/plugins**: token-budget accounting, baselines, a benchmark harness, an `evaluation-result-schema`, *and* its own `tests/` — "a skill pack has tests/evals" turned into shippable infrastructure ([[sources/openai--plugins#plugin-eval]]). It operationalizes this page's *"quantitative evals shipped with the artifact"* sub-mechanism in a third ecosystem. **Honest scope:** `plugin-eval` is the *evaluator* (it tests the output/budget of a pack); it is not verified to ship the rarest part — a per-skill **resolver eval** (does the skill *fire?*) — so it reinforces the eval half, not the resolver-eval half, and doesn't lift the count. — citation: [[sources/openai--plugins#plugin-eval]]
- *(counter-pressure sighting, 2026-06-10 — big pack, **no visible tests**)* [[artifacts/plugins/pm-os]] — the largest single-domain pack we've cataloged (235 skills, commercial, v2.2.1) ships taxonomy discipline (three-tier hierarchy, canonical names, machine-readable `registry/`) and versioned releases — but **no test suite, no LLM evals, no resolver eval** anywhere in the distributed zips. A paid product shipping untested where the open-source packs (gstack/gbrain/CE) ship ~1,000+ tests is a genuine data point *against* "a skill pack has tests" being universal — or evidence that commercial packs externalize QA to version-gated releases and buyer feedback. Worth tracking which reading wins. — citation: [[sources/prodmgmt-world--pm-os#root]], [[sources/prodmgmt-world--pm-os#registry]]
