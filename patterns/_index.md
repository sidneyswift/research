...
- *(new from pm-os deep-scan 2026-06-11; 1 example each, tracked on [[artifacts/plugins/pm-os]]):*
  - **imperative session-start directives** — passive hook nudges ("Consider running X") get silently absorbed by the LLM; emit an imperative directive mandating the first user-facing action + no-re-offer clause, with single-nudge arbitration between competing hooks and engagement-tapered cadence. Failure-tested (their issue #8). *Strongest promotion candidate — generalizes to any hook/heartbeat design.*
  - **prompt-corpus-to-skill conversion** — bulk-convert a prompt library into skills via a fixed skeleton (Required-Inputs / Instructions / Usage-Notes) + tool-normalized two-field frontmatter; accept `{{PLACEHOLDER}}` residue and near-duplicate clusters as the cost of scale (130/235 retain handlebars).
  - **canonical-log → projection → recall-packet memory** — append-only JSONL is the only source of truth; human-readable logs are regenerated projections; consumers get capped top-N packets with a `lookup_status` health enum, never raw dumps. Relative of [[patterns/behavioral/latent-vs-deterministic-split]] applied to memory.
...
- **passive hook nudges** *(1 example, self-documented — pm-os)* — "Consider running /tidy" phrasing in session-start hook output was "silently absorbed by the LLM as background context" (pm-os issue #8, fixed by the imperative-directive pattern above). Rare case where the creator documented their own anti-pattern + fix in-repo ([[sources/prodmgmt-world--pm-os#hook-directives]]).
- **marketing-prose count drift** *(1 example — pm-os)* — CI validates structural registries but not narrative claims, so README/manifest component counts drift from reality (235 vs 237 vs 214 skills). The fix their own tooling suggests: treat counts in prose like version strings — generated, never hand-written ([[sources/prodmgmt-world--pm-os#count-drift]]).
