...
## [2026-06-11] deep-scan | PM OS internals — chaining mechanics, memory model, telemetry

- Deep-scanned the local evidence copy (`sources/prodmgmt-world--pm-os-bundle/`, git-ignored) for the under-examined areas; ~30 candidate principles extracted (full report held locally with the licensed copy).
- **[[artifacts/plugins/pm-os]] substantially expanded:** exact workflow-chaining mechanism (inline 4-tuple steps: skill → path → goal → "output to carry forward"; confirm-per-step w/ end-to-end escape; framework menus; CI-synced registry mirror), tool-enforced skill house style (235/235 two-field frontmatter; 183 shared skeleton; **130 retain `{{HANDLEBARS}}` — the 211 reusable skills are a bulk-converted prompt corpus**), procedures-vs-reference split (Knowledge citation forced before opinions; metadata-routed Templates), 9-layer memory model (events.jsonl canonical → DECISION-LOG projection → capped recall packets w/ health enum; stale-yes consent rule; injection defense in the memory layer), and a new "Field notes: prompt-engineering mechanics" section (imperative directives after documented nudge failure #8, pre-flight self-audit, compaction-survival anchoring, **LLM-as-telemetry-client** — 5 fail-silent webhook curls in onboarding).
- **[[sources/prodmgmt-world--pm-os]]:** anchor map grew 7 anchors (workflow-chaining, skill-house-style, memory-schema, hook-directives, telemetry, templates-examples, count-drift).
- **Open questions resolved:** near-duplicate clusters confirmed (skill-coach/mastery/acquisition; 6 copies of one reference file); Cowork packaging mapped (drops bin/ + registry/; its prefix-less dropdown is *why* the `pm-os-` prefix exists).
- **[[patterns/_index]]:** 6 new 1-example proposed candidates (imperative session-start directives — strongest; prompt-corpus-to-skill conversion; canonical-log→projection→recall-packet memory; preview-confirm + stale-yes; metadata-routed content libraries; LLM-as-telemetry-client) and the **first two entries in Suspected anti-patterns** (passive hook nudges — creator-documented; marketing-prose count drift).
- ratchet: links +~15 · orphans +0 · anti-patterns 0→2 · proposed +6.

## [2026-06-11] ingest | pm-os deep-scan pass 3 — principles synthesis (rule engine, CI, onboarding, upgrade)

- Third pass over the licensed PM OS 2.2.1 bundle, synthesizing durable **principles** (the "why it works") on top of the prior inventory + mechanics passes. New `## Principles` section on [[artifacts/plugins/pm-os]]: 19 numbered principles across behavioral control (compliance-as-visible-output, placeholder sentinels, imperative directives, behavior-rules-outrank-style, no-decorative-subagents, scripted refusal edges), architecture (canonical-rules-with-projections / compaction-survival anchoring, registry-coupled changes, "structure is tested; behavior is not", memory-as-projection-stack-with-upgrade-boundary, citation-forcing-as-value-demo), engagement/growth (onboarding-as-activation-funnel, LLM-as-telemetry-client, engineered habit taper, ghost-written testimonials, switching-cost-reduction import, support-cost engineering), and multi-surface pipeline (design-names-for-the-weakest-UI, version-gates-buyer-state-migration) — plus a consolidated **Anti-patterns & weaknesses** subsection (self-reported compliance, fixed token tax, zero behavioral tests, bulk-converted corpus unevenness, count drift, docs referencing unshipped dev tooling, plaintext telemetry webhook, emoji-path brittleness). Absorbed and removed the interim "Field notes" section (DRY).
- **[[sources/prodmgmt-world--pm-os]]:** anchor map +5 (`#rule-engine`, `#ci-validators`, `#onboarding`, `#upgrade-skill`, `#agent-classification`).
- **[[patterns/_index]]:** pm-os proposed-candidate sub-list +2 (**compliance-self-report block**, **placeholder-sentinel setup gate**); deduped against the prior pass's entries rather than re-adding (a parallel pass had landed first — overlap resolved by merging, per the dedup-before-create rule).
- **What we'd steal** ledger +8 (pre-flight self-report ★, placeholder sentinels, prefilled onboarding drafts, ghost-written testimonials, telemetry mechanism, wrong-environment refusal, postmortem-embedded guards, agent-classification triage, weakest-UI naming).
- Doctor: 100/100, 90 pages, no broken links/orphans.
- ratchet: links +~12 · orphans +0 · patterns +0 confirmed (proposed +2) · anti-pattern candidates consolidated on the artifact page.
